...
router.get('/google/callback',
  ...
  (req, res) => {
    ...
    res.redirect(`https://login-adri-1.netlify.app?token=${token}&username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&role=${encodeURIComponent(role)}`);
  }
);
...
router.get('/google/register/callback', (req, res, next) => {
  ...
  passport.authenticate('google-register', { session: false }, async (err, user, info) => {
    if (err || !user) {
      return res.redirect('https://login-adri-1.netlify.app?error=Cuenta%20ya%20registrada');
    }
    ...
    res.redirect(`https://login-adri-1.netlify.app?token=${token}&username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&role=${encodeURIComponent(user.role)}`);
  })(req, res, next);
});
